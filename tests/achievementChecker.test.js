...
describe('条件判断逻辑 - checkCondition', () => {
  ...
  it('应返回false当数据为null时', () => {
    const condition = { level: 10 };
    const result = achievementChecker.checkCondition(condition, null);
    assert.strictEqual(result, false);
  });

  it('应返回false当数据为undefined时', () => {
    const condition = { level: 10 };
    const result = achievementChecker.checkCondition(condition, undefined);
    assert.strictEqual(result, false);
  });
});
...
describe('装备成就检查 - checkEquipmentAchievements', () => {
  ...
  it('应正确添加所有满足条件的装备成就', () => {
    ...
    assert.strictEqual(newAchievements.length, 3, '应有三个新成就');
    assert.strictEqual(player.props.cultivateDan, 125, '应获得125个培养丹（5+20+100）');
  });
